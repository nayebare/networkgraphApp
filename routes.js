...
var driver = neo4j.driver('bolt://52.87.215.121:32863', neo4j.auth.basic('neo4j', 'authority-compressions-spray'));
var session = driver.session();


router.post('/algo', function (req, res) {
// Which are most common routes
var query =
//   "CALL algo.betweenness.stream('Route', 'LOCATED_ON', {direction: 'BOTH'}) \
//    YIELD nodeId, centrality \
//    MATCH (c:Route) WHERE ID(c) = nodeId \
//    RETURN c.name AS Route, centrality \
//    ORDER BY centrality DESC \
//    LIMIT $limit";

   "CALL algo.degree.stream('Bstop', 'LOCATED_ON', {direction: 'incoming'})\
     YIELD nodeId, score \
    RETURN algo.asNode(nodeId).id AS Bstop, score AS bstops \
     ORDER BY bstops DESC \
     LIMIT $limit ";
